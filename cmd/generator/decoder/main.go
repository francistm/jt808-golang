...
import (
	...
	"github.com/francistm/jt808-golang/jt808/generator"
	"os"
)
...
func main() {
	...
	if err := generator.UpdateWorkingDir(); err != nil {
		panic(err)
	}
	...
	f.Func().
		...
		Block(
			...
			If(Id("messagePack").Dot("PackHeader").Dot("Package").Op("!=").Nil()).Block(
				Id("messagePack").Dot("PackBody").Op("=").New(Qual("github.com/francistm/jt808-golang/jt808/message", "PartialPackBody")),
			).Else().Block(
	...
	outputFile, err := os.OpenFile("jt808/decode_gen.go", os.O_CREATE | os.O_RDWR, 0644)
